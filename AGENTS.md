...
- Задачи вести через **GitHub Issues** (шаблоны: `.github/ISSUE_TEMPLATE/`). Полный workflow: `docs/GITHUB_WORKFLOW.md`.

**Правила для агента — когда создавать задачу:**
- По просьбе пользователя («создай задачу», «заведи issue на …») — выполнить `gh issue create --title "..." --body "..."` из корня репо (если `gh` установлен и авторизован; иначе — сформулировать задачу текстом и попросить создать вручную).
- Перед нетривиальной работой по формулировке пользователя без номера issue — создать issue, затем ветку `issue-N-short-name` и коммиты с `fixes #N`.
- Не создавать issue на каждую мелкую правку.

**Правила для агента — когда и как коммитить:**
- Коммитить логическими кусками (фича/фикс/рефакторинг). В сообщении: заголовок (feat/fix/docs), при работе по issue — `fixes #N` или `refs #N`.
- Работа по задаче: ветка от `master` вида `issue-N-short-name`, в коммитах `fixes #N`, пуш, PR с `fixes #N` в описании.
